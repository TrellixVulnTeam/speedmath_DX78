  ...
  socket.on("qotd_getQuestion", (token) => {
    jwt.verify(token, process.env['JWT_PRIVATE_KEY'], function(err, user) {
      if (err) {
        ...
        let releaseDate = new Date("August 1, 2022 00:00:00"); //time when first qotd problem was released, set to 4 AM because we want it to be in terms of EDT/EST and default js time is EST
