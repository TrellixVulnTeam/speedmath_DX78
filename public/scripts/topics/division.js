...
class Question {
  /*
    Algorithm to generate a division question:
    Generate a random number between a and b and another random number between c and d. These are 2 factors.
    Multiply these factors to get a product.
    The division question will be the product divided by the factor that's between a and b.
  */
  constructor(a, b, c, d) {
    let num1 = Math.floor(a + Math.random() * (b - a + 1)); //to generate a random number between a and b (inclusive)
    let num2 = Math.floor(c + Math.random() * (d - c + 1)); //to generate a random number between c and d (inclusive)
    let product = num1 * num2;

    this.question = `${product} ÷ ${num1}`;
    this.answer = product/num1;
  }
}
...
function generateQuestion(level) {
  switch (level) {
    case 1:
      return new Question(1, 5, 1, 5);
    case 2: 
      return new Question(1, 10, 1, 10);
    case 3:
      return new Question(1, 10, 10, 50);
    case 4:
      return new Question(1, 10, 10, 99);
    case 5:
      return new Question(10, 25, 10, 25);
    case 6:
      return new Question(10, 50, 10, 50);
    case 7:
      return new Question(10, 99, 10, 99);
    case 8: 
      return new Question(1, 10, 100, 999);
    case 9:
      return new Question(10, 50, 100, 999);
    case 10:
      return new Question(10, 99, 100, 999);
  }
}
...
if (token !== null) {
  loggedIn = true;
  socket.emit("getTopicPracticeStats", token, "division");
} else {
  loggedIn = false;
  ...
  function verifyAnswer() {
    ...
    if (answerField.value == question.answer) { //if correct:
      ...
      if (userStats.question == 10) { //level goes up if 10 questions of the previous level are done
        ...
        displayLevelScreen();
        if (loggedIn) {
          socket.emit("updateTopicPracticeStats", token, "division", userStats.level);
        }
        return;
      } else {
